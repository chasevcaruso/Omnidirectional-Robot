...
@TeleOp(name = "DRWA Right", group = "TeleOp")
public class DriveWithArmRightBox extends LinearOpMode {
    ...
    private enum ArmMovement {PAUSED, MOVE_UP, MOVE_DOWN_FOR_PICKUP, GRAB, OPEN, DUCK_DROP, STUCK}
    ...
    private final double ELBOW_UP = 0.6;
    ...
    private final double STUCK_SHOULDER = 0.9;
    private final double STUCK_ELBOW = 0.5;
    ...
    public void autonomousScore(){
        ...
        //strafe
        robot.startMove(0.1,0.1,0,0);
        while (robot.motorLeft.getCurrentPosition() > -1200 && opModeIsActive()) {
        }
        robot.startMove(0,0,0,0);

        //move left
        robot.resetDriveEncoders();
        robot.startMove(0,0.2,0,0);
        while (robot.motorLeft.getCurrentPosition() > -865 && opModeIsActive()){
        }
        robot.startMove(0,0,0,0);
        ...
        while (robot.motorLeft.getCurrentPosition() > -375 && opModeIsActive()) {
        }
        robot.startMove(0,0,0,0);
        ...
        controlArm();
        sleep(2000);
        armMovement = armMovement.STUCK;
        controlArm();

        //go backwards
        robot.resetDriveEncoders();
        robot.startMove(-0.1,0.0,0.01,0);
        while (robot.motorLeft.getCurrentPosition() > 100 && opModeIsActive()) {
        }
        robot.startMove(0,0,0,0);
    ...
    public void controlArm() {
        ...
        switch (armMovement) {
            ...
            case OPEN:
                ...
                break;
            case STUCK:
                elbow.setPosition(STUCK_ELBOW);
                shoulder.setPosition(STUCK_SHOULDER);
                elbow.update();
                shoulder.update();
                break;
            case DUCK_DROP:
